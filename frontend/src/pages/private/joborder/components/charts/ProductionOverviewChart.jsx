...
export function ProductionOverviewChart({ data, loading }) {
  ...
  useEffect(() => {
    ...
    const productTypeList = Array.from(uniqueProductTypes).map(productType => ({
      name: productType,
      key: productType, // Add key property that matches the data key
    ...
    // Set initial selected product type to "all"
  ...
  if (!data || !data.monthlyData || data.monthlyData.length === 0) {
    return (
      <CustomChart
        ...
        loading={loading}
        icon={TrendingUp}
        emptyMessage={loading ? "Loading data..." : "No production data available"}
  ...
  if (productTypes.length === 0) {
    return (
      <CustomChart
        ...
        loading={loading}
        icon={TrendingUp}
        emptyMessage="No product type data available for the selected period"
        initialChartType="line"
      />
    );
  }
      ...
      <CardHeader>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <CardTitle className="flex items-center gap-2">
            ...
            Production Overview by Product Type
          </CardTitle>

        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2">
            <InfoCard
              icon={Calendar}
              label="Created At"
              value={formatDate(unit?.createdAt, "long")}
            />
            <InfoCard
              icon={Calendar}
              label="Updated At"
              value={formatDate(unit?.updatedAt, "long") || "Not updated yet"}
            />
          </div>
        </div>
      </div>
...
const UnitDetailsDialog = ({ isOpen, onClose, unit }) => {
  if (!unit) return null;

  return (
    <ViewDetailsDialog open={isOpen} onClose={onClose} title="Unit Details">
      <UnitContent unit={unit} />
    </ViewDetailsDialog>
  );
};

// Add UnitDetails export for backward compatibility
const UnitDetails = ({ unit }) => (
  <UnitContent unit={unit} />
);

export { UnitDetailsDialog, UnitDetails };

...
import { Box, Calendar, Info } from "lucide-react";
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2">
            <InfoCard
              icon={Calendar}
              label="Created At"
              value={formatDate(size?.createdAt, "long")}
            />
            <InfoCard
              icon={Calendar}
              label="Updated At"
              value={formatDate(size?.updatedAt, "long") || "Not updated yet"}
            />
          </div>
        </div>
      </div>
...
const SizeDetailsDialog = ({ isOpen, onClose, size }) => {
  if (!size) return null;

  return (
    <ViewDetailsDialog open={isOpen} onClose={onClose} title="Size Details">
      <SizeContent size={size} />
    </ViewDetailsDialog>
  );
};

// Add SizeDetails export for backward compatibility
const SizeDetails = ({ size }) => <SizeContent size={size} />;

export { SizeDetailsDialog, SizeDetails };
